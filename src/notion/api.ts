...
export async function createPage(args: {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Failed to create page in database ${args.parentDatabaseId}: ${response.status} ${text}`);
  }
  return await response.json();
}

export async function getPagePropertyItems(
  pageId: string,
  propId: string,
...
export async function getRelationIdsFromPageProperty(pageId: string, propId: string): Promise<string[]> {
  const ids: string[] = [];
...
export async function getSingleRelationIdFromPageProperty(pageId: string, propId: string): Promise<string | null> {
